...
class CronAudit(models.Model):
    ...
    num_failures = fields.Integer('Number of Failures', related='cron_id.failure_count', store=True)
    first_failure_date = fields.Char('First Failure Date', compute='_compute_first_failure_date', store=True)
    ...
    @api.depends('cron_id.first_failure_date')
    def _compute_first_failure_date(self):
        for cron in self:
            cron.first_failure_date = str(cron.cron_id.first_failure_date) + " UTC" if cron.cron_id.first_failure_date else None
